...
import matplotlib
import pandas as pd
from utils.cfg import budget, start_period, end_period
...
def env_check() -> None | NoReturn:
    if not os.path.exists(".env"):
        ...
        raise FileNotFoundError(
            "Please add your API key to the '.env' file. You can generate it here: https://www.alphavantage.co/support/#api-key"
        )
...
def main(args: argparse.Namespace) -> None:
    if args.api:
        ...
        launch_optimization(args)


def launch_optimization(args):
    with open(f"{input_dir}/{args.file}", "r") as f:
        data = "\n".join(f.readlines())
    dt = DataCollector.loads(data)
    portfolio = Portfolio(budget)
    dates = date_range(start_period, end_period)
    solutions = []
    for solution in time_scaled_optimization(dt, dates, portfolio):
...
if __name__ == "__main__":
    ...
    _argparse = argparse.ArgumentParser(
        description="Collects and analyzes data from various sources."
    )
